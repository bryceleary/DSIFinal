...
Prior to formally testing our hypothesis ,we executed exploratory `r ggplot`s to view the shape of our explanatory and response variables (figure placeholder). This initial view led us to consider a basic linear regression to fit our analysis and knowledge of statistical methods.

This regression indicated that multilateral donor flow and export flows are the two main influences on our response variables, `r adesa` and `r depth`.
...
To clean this dataset, we kept `r country` and `r country_code` as the unique identification variables for each observation. This decision relies on the assumption that country would be the. main variable by which we would need to use numerical identification as fallback.

During the cleaning and merging process, (placeholder for Ivory coast) stood out as an observation that was inconsistently spelled across the distinct datasets. To avoid editing the excel sheet, we reconciled this inconsistent spelling by editing the `r country` column form a factor to character. Then, we replaced the existing spelling with NA. After this, we replaced NA with the alternative spelling.
...
The linear regression in our study excludes a flow type, `r private`.The `r private` observations were omitted from the regression due to their low n-size and minimal contribution to the exploratory Bayesian regression method mentioned above.
